...
def load_data():
    try:
        ...
        st.error("❌ Error: movies_cleaned.csv not found. Please ensure the data file is in the app directory.")
        st.stop()
...
def load_model():
    try:
        ...
        st.info("Required files: movieiq_rf_model.joblib and genre_encoder.joblib")
        st.stop()
